...
/// <summary>
/// Rotate tool view model that delegates execution to the shared job framework.
/// </summary>
public sealed partial class RotateViewModel : PageViewModel
{
    private readonly IJobExecutor _jobExecutor;
    private readonly IPdfEngine _pdfEngine;
    private readonly IFileDialogService _fileDialogService;
    private readonly ILogger<RotateViewModel> _logger;
    ...
    public RotateViewModel(
        ...
        ILogger<RotateViewModel> logger)
    {
        ...
        _outputFileName = "Rotated.pdf";
        _outputFile = "Rotated.pdf";
    ...
    /// <inheritdoc />
    public override string IconKey => "Rotate";

    /// <summary>The file selected for rotation.</summary>
    [ObservableProperty]
    ...
    /// <summary>Pages to rotate, e.g. "1-3, 5, 7-".</summary>
    [ObservableProperty]
    private string _pageRangeText = string.Empty;

    /// <summary>Rotation applied to the selected pages. The engine rejects <see cref="Rotation.None"/>.</summary>
    [ObservableProperty]
    private Rotation _rotation = Rotation.Clockwise90;
    ...
    /// <summary>Elapsed time for the active or most recent rotation.</summary>
    [ObservableProperty]
    ...
    /// <summary>Lets the user pick a PDF file to rotate.</summary>
    ...
    /// <summary>Executes the rotate job.</summary>
    public async Task ExecuteRotateAsync()
    {
        ...
        LastSummary = null;
        ProgressPercent = 0;
        ProgressMessage = "Preparing to rotate pages...";
        ...
        var request = new RotatePagesJobRequest(InputFile, OutputFile, ranges, Rotation);
        ...
        try
        {
            var result = await _jobExecutor.ExecuteAsync(
                ...
                {
                    var rotateJob = new RotatePagesJob(_pdfEngine);
                    return rotateJob.ExecuteAsync(job, context, cancellationToken);
                },
            ...
            ProgressMessage = result.Succeeded ? "Rotation completed." : result.ErrorMessage ?? "Rotation failed.";
            ElapsedText = FormatElapsed(result.Elapsed);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Rotate job failed");
            IsExecutionComplete = true;
            IsSuccess = false;
            LastError = "Rotation failed unexpectedly.";
            ProgressMessage = "Rotation failed unexpectedly.";
        }
        finally
        {
            _elapsedTimer.Stop();
    ...
    /// <summary>Cancels the currently running rotate job.</summary>
    ...
    /// <summary>Resets the workflow so the user can rotate another file.</summary>
    public void RotateAnother()
    {
        ...
        SetOutputFileName("Rotated.pdf", markCustom: false);
        RefreshExecutionState();
    }
    ...
    [RelayCommand]
    internal async Task ExecuteRotateCommand() => await ExecuteRotateAsync().ConfigureAwait(true);
    ...
    [RelayCommand]
    internal void RotateAnotherCommand() => RotateAnother();
    ...
    private string? GetValidationMessage()
    {
        ...
        {
            return "Choose a PDF file to rotate.";
        }
    ...
    private void SyncOutputPartsFromFile(string value)
    {
        ...
        try
        {
            ...
            OutputFileName = string.IsNullOrWhiteSpace(fileName) ? "Rotated.pdf" : fileName;
        }
        finally
        {
    ...
    private static string SuggestOutputFileName(string inputFile)
    {
        ...
        return string.IsNullOrWhiteSpace(stem) ? "Rotated.pdf" : $"{stem}-rotated.pdf";
    }
