...
public sealed class ExtractViewModelTests
{
    [Fact]
    public void Constructor_initializes_with_expected_state()
    {
        ...
        Assert.Equal("Extracted.pdf", viewModel.OutputFileName);
        Assert.Equal("Choose a PDF file to extract pages from.", viewModel.ValidationMessage);
    }
    ...
    public async Task Selecting_file_reads_page_count_and_updates_suggested_name()
    {
        ...
        Assert.Equal("Alpha-extracted.pdf", viewModel.OutputFileName);
    }
    ...
    [Fact]
    public async Task Execute_extract_sets_complete_state_when_job_succeeds()
    {
        using var fixture = new PdfFixture();
        var alpha = fixture.CreateFile("Alpha.pdf");
        ...
        executor.Setup(x => x.ExecuteAsync(It.IsAny<ExtractPagesJobRequest>(), It.IsAny<Func<ExtractPagesJobRequest, JobExecutionContext, CancellationToken, Task<JobExecutionResult>>>(), It.IsAny<IProgress<JobProgress>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new JobExecutionResult(JobOutcome.Succeeded, null, "Extracted 2 page(s) into Alpha-extracted.pdf", Path.Combine(outputDirectory, "Alpha-extracted.pdf"), TimeSpan.FromSeconds(2)));
        ...
        await viewModel.ExecuteExtractAsync();

        Assert.True(viewModel.IsExecutionComplete);
        Assert.True(viewModel.IsSuccess);
        Assert.Equal("Extracted 2 page(s) into Alpha-extracted.pdf", viewModel.LastSummary);
        Assert.Equal("Extraction completed.", viewModel.ProgressMessage);
    }

    [Fact]
    public async Task Extract_another_resets_workflow_state()
    {
        using var fixture = new PdfFixture();
        var alpha = fixture.CreateFile("Alpha.pdf");
    ...
    private static ExtractViewModel CreateViewModel(IJobExecutor? executor = null, IPdfEngine? pdfEngine = null)
    {
        ...
        return new ExtractViewModel(
            ...
            NullLogger<ExtractViewModel>.Instance);
    }
