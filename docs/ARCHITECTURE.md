...
â€” entirely offline, no accounts, no telemetry, with a UI that feels like a
...
```
Acroball.Desktop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º composition root; only assembly seeing everything
   â”‚                â”‚
Acroball.UI      Acroball.Infrastructure
   â”‚                â”‚
   â””â”€â”€â–º Acroball.Application â—„â”€â”€â”˜        contracts + request records
                â”‚
...
2. Domain and Application contain no framework or I/O types â€” they compile
...
(AppDomain + unobserved task exceptions â†’ log file), then starts Avalonia.
...
cross-fade). Pages are created by `PageFactory` (id â†’ view model; Home and
...
table â€” no reflection, trim-safe (ADR-0003/0005). Cross-page navigation uses
...
- **Palette.axaml** â€” semantic brushes only (`Q.Brush.Surface`,
  `Q.Brush.TextSecondary`, `Q.Brush.Accent`, â€¦) in Light ("warm paper":
...
- **Tokens.axaml** â€” corner radii (6/8/12).
- **Typography.axaml** â€” Inter, compact desktop ramp: h1 24/semibold,
...
- **Icons.axaml** â€” 26 Fluent System Icons as `StreamGeometry` resources
  keyed `Icon.*`; view models refer to icons by string key, resolved by
  `IconKeyToGeometryConverter`.
- **Controls.axaml** â€” button variants (`primary`, `ghost`, `danger`),
...
Motion defaults: 120â€“180 ms, ease-out, opacity/transform only.

Theme switching: `IThemeService` (Application) â†’ `ThemeService` (UI) sets
...
- Infrastructure tests: real files in per-test temp directories via
  `AppPaths(tempDir)` â€” no mocking of the filesystem.
- Engine integration tests generate fixture PDFs with PDFsharp itself and
  ...
  assertions never reAcroball text rendering or fonts â€” the suite stays green
...
excluded *by decision*: the plugin system reAcroballs runtime assembly loading.
...
Ctrl+1â€¦9 navigation), visible focus indication, AutomationProperties on
interactive elements, contrast â‰¥ 4.5:1 for text in both themes, respect
reduced-motion by keeping animation opacity/transform-only and short.

## Privacy

Everything is local. No network calls exist in M1â€“M5 at all; M6 adds exactly
one â€” the self-update check â€” which is user-visible and disableable.
