...
- **Acroball.Domain** — value types and rules (page ranges, rotation, metadata,
  permissions). Zero dependencies.
- **Acroball.Application** — use-case contracts (`IPdfEngine`,
  `IPdfRenderService`, settings/recent/theme/update services) and request
  records. Depends only on Domain.
- **Acroball.Infrastructure** — persistence, logging, updates and (from M2) PDF
  backends. Depends on Application.
- **Acroball.UI** — Avalonia views, view models, theme. Depends on Application;
  never on Infrastructure.
- **Acroball.Desktop** — entry point and the *only* composition root; the only
